defmodule Karn.AI.ModelsTest do
  ...
  describe "valid/1" do
    ...
    test "invalid format" do
      assert Models.valid("aa") == {:error, "model must be in format provider:model like #{"google:gemini-2.0-flash"}"}
    end
    ...
    test "valid provider and invalid model" do
      assert Models.valid("google:invalid_model") == {:error,"Only gemini-flash-latest\ngemini-embedding-001\ngemini-2.5-pro\ngemini-2.5-flash-preview-05-20\ngemini-2.5-flash\ngemini-2.0-flash-lite\ngemini-2.0-flash\n are supported. You provided: invalid_model"} 
    end
  end
end
